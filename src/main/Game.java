...
public class Game extends BasicGame{
	private final Color background = new Color(130,131,130);
	...
	public void init(GameContainer gc) throws SlickException {
		...
		objects[6] = new GameObject(new Rectangle(150,600,90,50),Color.black);
...
//					System.out.println("Collided right");
				...
				if(p.getSpeedY() > 0 && p.collideDown(obj) && Math.abs(p.getY() - (obj.getY() + obj.shape.getHeight() * -1)) < 20){ // Player collided with an object below
					collisionObject2 = obj;
					dir2 = -1;
					p.setSpeedY(0);
					if(yGravity > 0){
...
//					System.out.println("Collided down");
				} else if(p.getSpeedY() < 0 && p.collideUp(obj) && Math.abs(p.getY() - (obj.getY() + obj.shape.getHeight() * 1)) < 20){ // Player collided with an object above
					p.setSpeedY(0);
					if(yGravity < 0){
						p.setJumping(false);
					}
...
//			System.out.println("Dir: " + dir);
			p.setX(collisionObject.getX() + collisionObject.shape.getWidth() * dir);
...
//			System.out.println("Dir2: " + dir2);
			p.setY(collisionObject2.getY() + collisionObject2.shape.getHeight() * dir2);
